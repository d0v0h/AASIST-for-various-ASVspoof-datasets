...
def load_cm_scores_keys(cm_scores_file, cm_keys_file, default_index=g_filename_tag):
    ...
    # load tsv files
    cm_scores_pd = pd.read_csv(cm_scores_file, sep=' ', header=None,
                               usecols=[1, 2],
                               names=[g_filename_tag, g_cm_score_tag])
    cm_keys_pd = pd.read_csv(cm_keys_file, sep=' ', header=None,
                             usecols=[1, 8],
                             names=[g_filename_tag, g_cm_label_tag], nrows=cm_scores_pd.shape[0])
    
    cm_scores_pd = cm_scores_pd.set_index(default_index)
    cm_keys_pd = cm_keys_pd.set_index(default_index)
